...
## Vacancies
There is usually little that requires changing on this page. If however, a vacancy in the group opens up then the vacancy can be listed. There are many ways of doing this, but in following the design language for the rest of the site, the `box` class could be used listing the vacancy. An example entry for a PostDoc vacancy could look like:
